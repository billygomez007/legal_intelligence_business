...
| Same-document succession; person-verified critical metadata; the provenance-attestation record and the approval/publication gate that requires both | `legal/corpus` | corpus migration `0004` ([ADR-0008](../adr/0008-integrity-hardening-stages-0-5.md)) |
| The requester-is-not-approver rule; the checked writer of the provenance attestation   | `legal/ingestion`    | ingestion migration `0002`               |
...
Ingestion depends on corpus and platform; nothing depends on ingestion (enforced by the architecture rules). **Ingestion installs no trigger on a corpus table.** It defines exactly one `SECURITY DEFINER` function, `ingestion.attest_provenance` (section 8), the sole writer of a corpus-owned record it may not write itself; the function is inventoried and its properties are enforced by the structural guardrails. Ingestion otherwise reaches the corpus through `corpusStore` and reads rights through `corpus.rights_decision_in_force`.
...
- **HTML:** parsed as data with `parse5`; scripts, styles, frames, embedded objects, templates and comments are never extracted; text a reader cannot see (`hidden`, `display:none`, `visibility:hidden`) is dropped **and reported** (`html_hidden_content_dropped`). An ordinary `style` attribute is not a reason to drop text. Traversal is iterative and node-bounded, so deep nesting and markup bombs end in a typed failure. Parsing runs in a **worker thread with a 10-second deadline and a memory ceiling**, because tree construction is CPU-bound and, on crafted markup, quadratic (found in independent review: 142 seconds for a 4 MiB document, on the event loop, holding the job lock); on timeout the thread is terminated and the document ends as `extraction_failed`.
...
**Provenance attestation.** The corpus will not approve or publish a version until a provenance attestation exists for it, and the corpus cannot read ingestion tables to check ingestion's evidence. So the attestation is a corpus-owned, append-only record (`corpus.version_provenance_attestations`: the version, source, content checksum and pipeline version, the attestation type, a pointer to the evidence, the system that attested, the person who requested the acquisition, and a time the database assigns) that ingestion writes only after its own checks. **No runtime role can insert it.** The pipeline calls `ingestion.attest_provenance(job)` in the hand-off transaction; that function takes a *job*, requires that it passed the hand-off gate, re-checks the rights and derives every value from the evidence it verified, so it cannot be made to attest something ingestion did not check. A composite foreign key makes an attestation naming the wrong source, content or pipeline version impossible to record, and a deferred trigger stops a job from ending its transaction awaiting review without one. See [ADR-0008](../adr/0008-integrity-hardening-stages-0-5.md).
...
Two further rules apply to approval (ADR-0008). **The person who requested the ingestion cannot approve it**, even holding both permissions: the database refuses it (`ingestion.requester_cannot_approve`) and `decide` checks first for a clear error. It applies to approval only; a requester may still inspect, reject and hold. And **the publish-critical metadata must have been verified by a person**: the packet lists each critical field for the kind of document (a case needs a court and a decision date; legislation does not) with the fingerprint of the value shown; `verifyMetadata` records a person's verification of it, bound to that exact value; `recordCaseDetails` lets a person record the court and date from the source, which extraction does not write. Approval (and publication) is refused until every required field has a current verification and provenance is attested. Machine evidence is never edited or marked verified.
...
- **`SECURITY DEFINER`** is inventoried and enforced by guardrails. Ingestion has exactly one, `ingestion.attest_provenance`: search path pinned, static SQL, reads only ingestion tables, executable by the ingest role alone.
...
PDF and OCR are not implemented. There is no Ghanaian parser and no pattern-based citation detection: they wait for representative lawful samples and verified citation conventions, and no format is invented. A review-class failure cannot be resumed by a person, and there is no override for a low-quality extraction; accepting one will need an explicit exception workflow with a second person's approval before the material becomes publishable or searchable (designed, not built). Artifacts above 4 MiB are refused. There is no worker daemon, only `runReady`. Raw artifacts of a revoked source are retained as restricted evidence; there is no purge (designed, not built) and no audit of reads of raw storage. Requester-is-not-approver is enforced at the ingestion mirror of a review decision: a service holding the data-ops role could still approve directly in the corpus, which cannot see ingestion. Two accounts held by one person defeat every separation of duties. There is a reviewer-facing operation to record a case's court and decision date but none for a legislation identifier, and metadata corrections after approval are not re-verified (they block publication until the version goes back through review); an audited correction workflow is future work. Full details, and what was deferred and why, are in the [review document](../reviews/stage-5-existing-implementation-review.md).
